...
public class App extends Application {
  ...
  private static final Position GRAPH_CENTRAL =
          new Position(GRAPH_BORDER + GRAPH_RADIUS, GRAPH_BORDER + GRAPH_RADIUS);
  private static final int NODE_RADIUS = 20;          // 节点的半径
  ...
  public void start(Stage stage) {
    ...
    stage.setTitle("App");
    // 按钮
    final Label lblCurrentFunction = new Label("当前功能：加载文件");
    final Button btnLoadFile = new Button("加载文件");
    final Button btnShowGraph = new Button("展示有向图");
    final Button btnQueryBridgeWords = new Button("查询桥接词");
    final Button btnGenerateNewWord = new Button("根据桥接词生成新文本");
    final Button btnShortestPath = new Button("计算两个词之间的最短路径");
    final Button btnRandomWalk = new Button("随机游走");
    ...
    final Button btnSaveImage = new Button("保存图片");
    final Button btnRestoreImage = new Button("还原图片");
    pneGraph = new Pane();
    ...
    // 查询桥接词
    final GridPane pneQueryBridgeWords = getPneQueryBridgeWords();
    // 根据桥接词生成新文本
    final GridPane pneGenerateNewWord = getPneGenerateNewWord();
    // 计算两个词之间的最短路径
    final GridPane pneShortestPath = getPneShortestPath();
    // 随机游走
    final GridPane pneRandomWalk = getPneRandomWalk(stage);
    ...
    buttonsVbox.getChildren().addAll(
            lblCurrentFunction, btnLoadFile, btnShowGraph, btnQueryBridgeWords,
            btnGenerateNewWord, btnShortestPath, btnRandomWalk);
    ...
    btnLoadFile.setOnAction(
            e -> changePane(right, pneLoadFile, false, lblCurrentFunction, btnLoadFile.getText()));
    ...
    btnQueryBridgeWords.setOnAction(
            e -> changePane(right, pneQueryBridgeWords, true,
                    lblCurrentFunction, btnQueryBridgeWords.getText()));
    btnGenerateNewWord.setOnAction(
            e -> changePane(right, pneGenerateNewWord, true,
                    lblCurrentFunction, btnGenerateNewWord.getText()));
    btnShortestPath.setOnAction(
            e -> changePane(right, pneShortestPath, true,
                    lblCurrentFunction, btnShortestPath.getText()));
    btnRandomWalk.setOnAction(
            e -> changePane(right, pneRandomWalk, true,
                    lblCurrentFunction, btnRandomWalk.getText()));
  ...
  private GridPane getPneRandomWalk(Stage stage) {
    ...
    final Button btnRandomWalkExecute = new Button("执行");
    final Button btnRandomWalkInterrupt = new Button("中断");
    final Button btnRandomWalkSaveResult = new Button("保存结果");
    ...
    btnRandomWalkSaveResult.setOnAction(
            e -> saveFile(txtRandomWalkResult.getText(), stage, fileSaveTxt));
  ...
  private GridPane getPneGenerateNewWord() {
    ...
    final Label lblGenerateNewWordWordInput = new Label("输入文本：");
  ...
  private GridPane getPneShortestPath() {
    ...
    final Label lblShortestPathWord1 = new Label("词1：");
    final Label lblShortestPathWord2 = new Label("词2：");
    ...
    final Label lblShortestPathLength = new Label("最短路径长度：");
    ...
    pneInit(pneShortestPath, lblShortestPathWord1,
            lblShortestPathWord2, txtShortestPathWord1,
            txtShortestPathWord2, btnShortestPathExecute, txtShortestPathResult);
    pneShortestPath.add(lblShortestPathLength, 0, 3);
    pneShortestPath.add(txtShortestPathLength, 1, 3);
    return pneShortestPath;
  }

  private void pneInit(GridPane pneShortestPath, Label lblShortestPathWord1,
                       Label lblShortestPathWord2, TextField txtShortestPathWord1,
                       TextField txtShortestPathWord2, Button btnShortestPathExecute,
                       TextField txtShortestPathResult) {
  ...
  private GridPane getPneQueryBridgeWords() {
    ...
    pneInit(pneQueryBridgeWords, lblQueryBridgeWordsWord1,
            lblQueryBridgeWordsWord2, txtQueryBridgeWordsWord1,
            txtQueryBridgeWordsWord2, btnQueryBridgeWordsExecute, txtQueryBridgeWordsResult);
    return pneQueryBridgeWords;
  }

  private void changePane(VBox right, Pane targetPane,
                          boolean check, Label lblShow, String wordShow) {
    // 使用按钮切换功能时，页面的变化
    if (check && !isLoaded) {
  ...
  private String randomWalk(TextField result) {
    /* 随机游走（启动线程） */
    myThread = new Thread(new RandomWalkThread(this.word2id,
            this.id2word, this.graph.graph, result));
    myThread.start();
    return null;
  }
  ...
  private String calcShortestPath(String word1, String word2) {
    ...
    if (min == max) {
      ...
      List<Integer> pathIndexList = getPathIndex(this.minPath,
              word2id.get(word1), word2id.get(word2));
  ...
  private void showDirectedGraph(AdjacencyList graph, Pane pane, List<Integer> marked) {
    ...
    for (int i = 0; i < graph.size; i++) {
      for (Edge edge : graph.graph.get(i)) {
        ...
        Line mainLine = new Line(arrowPosition[0].x(),
                arrowPosition[0].y(),
                arrowPosition[1].x(),
                arrowPosition[1].y());
        Line arrowLineL = new Line(arrowPosition[1].x(),
                arrowPosition[1].y(),
                arrowPosition[2].x(),
                arrowPosition[2].y());
        Line arrowLineR = new Line(arrowPosition[1].x(),
                arrowPosition[1].y(),
                arrowPosition[3].x(),
                arrowPosition[3].y());
        Label lblWeight = new Label(String.valueOf(edge.weight));
        lblWeight.setTranslateX(arrowPosition[0].x()
                 + (arrowPosition[1].x() - arrowPosition[0].x()) * 0.3);
        lblWeight.setTranslateY(arrowPosition[0].y()
                 + (arrowPosition[1].y() - arrowPosition[0].y()) * 0.3);
  ...
  private Position calcNodePosition(int id) {
    ...
    return new Position(GRAPH_CENTRAL.x() + (int) (GRAPH_RADIUS * Math.cos(angle)),
            GRAPH_CENTRAL.y() + (int) (GRAPH_RADIUS * Math.sin(angle)));
  }
